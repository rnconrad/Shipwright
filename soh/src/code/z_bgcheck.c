...
void BgCheck_DrawSceneInfoStaticCollision(GlobalContext* globalCtx, CollisionContext* colCtx) {
    CollisionPoly* curPoly;
    CollisionPoly* polyList;
    SSNode* curNode;
    s32 polyId;
        ...
        for (sector.y = sectorStart.y; sector.y < sectorEnd.y; sector.y++) {
            for (sector.x = sectorStart.x; sector.x < sectorEnd.x; sector.x++) {
                ...
                while (true) {
                    ...
                    s32 polyFlags = func_80041DB8(&globalCtx->colCtx, curPoly, BGCHECK_SCENE);
                    if (polyFlags & (0x2 | 0x8 | 0x10)) {
                        gDPSetOverrideColor(POLY_OPA_DISP++, 0, 0, 0xA0, 0, (polyFlags & 0xFF), 255);
                        BgCheck_DrawStaticPoly(globalCtx, colCtx, curPoly, 0xFF, 0xFF, 0xFF);
                    }
