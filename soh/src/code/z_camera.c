...
s32 Camera_Blind(Camera* camera) {
    ...
    camera->dist = eyeAdjustment.r = 250;
...
s32 Camera_Normal1(Camera* camera) {
    if (CVar_GetS32("gBlind_NoCameraTurn", 0) != 0) {
        Camera_Blind(camera);
        return;
    }
    ...
    if (anim->startSwingTimer <= 0) {
        eyeAdjustment.yaw = CVar_GetS32("gBlind_NoCameraTurn", 0) != 0 ? atEyeNextGeo.yaw
                                                              : Camera_LERPCeilS(anim->swingYawTarget, atEyeNextGeo.yaw,
                                                                                 1.0f / camera->yawUpdateRateInv, 0xA);
        ...
        eyeAdjustment.yaw =
            CVar_GetS32("gBlind_NoCameraTurn", 0) != 0
                ? atEyeNextGeo.yaw
